var Tadpole = function () {
    ...
    this.draw = function (context) {
        ...
        // ctx.beginPath();
        // ctx.arc(tadpole.x, tadpole.y, 50, 0, 2 * Math.PI, false);
        // ctx.closePath();
        // ctx.fill();
        // console.log('TX = ' + tadpole.x + ' TY = ' + tadpole.y);
    ...
    this.shoot = function (x, y, angle) {
        tadpole.bullets.push(new Bullet(x, y, angle));
    };
    ...
    var drawBullets = function (context) {
        for (var i in tadpole.bullets) {
            tadpole.bullets[i].draw(context);
        }
    };

    var drawCrash = function (context) {
        if (tadpole.crash) {
            context.fillStyle = 'rgba(255,255,255, 0.8)';
            context.beginPath();
            context.arc(tadpole.x, tadpole.y, 50, 0, 2 * Math.PI, false);
            context.closePath();
            context.fill();
        }
    };
