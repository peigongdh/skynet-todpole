var App = function (aSettings, aCanvas) {
    ...
    app.onSocketOpen = function (e) {
        ...
        // console.log('Socket opened finish');
    ...
    app.sendMessage = function (msg) {
        ...
        if (messageQuota > 0) {
            messageQuota--;
            webSocketService.sendMessage(msg, model.userTadpole);
        }
    ...
    var collideUpdate = function() {
        for (var i in model.tadpoles) {
            ...
            for (var j in model.tadpoles[i].bullets) {
                model.tadpoles[i].bullets[j].collideUpdate(model);
            }
        }
    };
